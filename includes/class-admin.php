...
class WPNCR_Admin {
	...
	private function __construct() {
		...
		add_action( 'wp_ajax_wpncr_search_content', array( $this, 'ajax_search_content' ) );
		add_action( 'wp_ajax_wpncr_search_products', array( $this, 'ajax_search_products' ) );
		add_action( 'wp_ajax_wpncr_add_relation', array( $this, 'ajax_add_relation' ) );
		add_action( 'wp_ajax_wpncr_remove_relation', array( $this, 'ajax_remove_relation' ) );
	}
	...
	public function render_meta_box( $post ) {
		...
		<div id="wpncr-relationships">
			<div class="wpncr-relation-types">
				<?php foreach ( $relation_types as $type => $type_info ) : 
				...
				?>
					<div class="wpncr-relation-type" data-type="<?php echo esc_attr( $type ); ?>">
							...
							<?php if ( isset( $grouped[ $type ] ) ) : ?>
								...
								?>
									<div class="wpncr-relation-item" data-related-id="<?php echo esc_attr( $rel->to_id ); ?>">
										...
										<button 
											type="button" 
											class="button wpncr-remove-relation" 
						...
						<div class="wpncr-add-relation">
							<label for="wpncr-search-<?php echo esc_attr( $type ); ?>" class="screen-reader-text">
								<?php
							...
							<input 
								type="text" 
								id="wpncr-search-<?php echo esc_attr( $type ); ?>"
								class="wpncr-search-input" 
								placeholder="<?php esc_attr_e( 'Search content...', 'native-content-relationships' ); ?>" 
								data-relation-type="<?php echo esc_attr( $type ); ?>"
								aria-describedby="wpncr-search-desc-<?php echo esc_attr( $type ); ?>"
							/>
							<p id="wpncr-search-desc-<?php echo esc_attr( $type ); ?>" class="screen-reader-text">
								<?php
								/* translators: %s: Relationship type label */
								printf( esc_html__( 'Type to search for content to relate with type %s', 'native-content-relationships' ), esc_html( $type_label ) );
								?>
							</p>
							<div class="wpncr-search-results" style="display: none;" role="listbox" aria-label="<?php esc_attr_e( 'Search results', 'native-content-relationships' ); ?>"></div>
						</div>
					</div>
				<?php endforeach; ?>
			</div>
	...
	public function enqueue_scripts( $hook ) {
		...
		wp_enqueue_script(
			'wpncr-admin',
			WPNCR_PLUGIN_URL . 'assets/js/admin.js',
		...
		wp_enqueue_style(
			'wpncr-admin',
			WPNCR_PLUGIN_URL . 'assets/css/admin.css',
			array(),
			WPNCR_VERSION
		);
		
		wp_localize_script( 'wpncr-admin', 'wpncrData', array(
			'ajaxUrl' => admin_url( 'admin-ajax.php' ),
			'nonce'   => wp_create_nonce( 'wpncr_ajax' ),
			'strings'  => array(
	...
	public function ajax_search_products() {
		check_ajax_referer( 'wpncr_ajax', 'nonce' );
	...
	public function save_relationships( $post_id, $post ) {
		// Verify nonce
		if ( ! isset( $_POST['wpncr_nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['wpncr_nonce'] ) ), 'wpncr_save_relationships' ) ) {
			return;
		}
	...
	public function ajax_add_relation() {
		...
		$result = WPNCR_API::add_relation( $from_id, $to_id, $relation_type );
	...
	public function ajax_remove_relation() {
		...
		$result = WPNCR_API::remove_relation( $from_id, $to_id, $relation_type );
