...
class WPNCR_Integrity {
	...
	public function maybe_run_integrity_check() {
		...
		$last_check = get_option( 'wpncr_last_integrity_check', 0 );
		...
		if ( $results['cleaned'] > 0 ) {
			...
			set_transient( 'wpncr_integrity_notice', $results, HOUR_IN_SECONDS );
		}
		
		update_option( 'wpncr_last_integrity_check', time() );
	}
	...
	public function run_integrity_check() {
		...
		foreach ( $all_relations as $rel ) {
			...
			if ( ! WPNCR_Relation_Types::exists( $rel->type ) ) {
				$to_delete[] = $rel->id;
				$issues['invalid']++;
				$cleaned++;
				continue;
			...
			$type_info = WPNCR_Relation_Types::get_type( $rel->type );
		...
		if ( ! empty( $to_delete ) ) {
			...
			// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching,WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- IDs are sanitized with absint()
			$wpdb->query( "DELETE FROM `{$wpdb->prefix}content_relations` WHERE id IN ({$ids_escaped})" );
		}
	...
	public static function show_integrity_notice() {
		$notice = get_transient( 'wpncr_integrity_notice' );
		
		if ( ! $notice || $notice['cleaned'] === 0 ) {
			return;
		}
		
		delete_transient( 'wpncr_integrity_notice' );
...
// Show notice
add_action( 'admin_notices', array( 'WPNCR_Integrity', 'show_integrity_notice' ) );
