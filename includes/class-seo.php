...
class WPNCR_SEO {
	...
	private function init() {
		...
		add_filter( 'wpncr_seo_internal_links', array( $this, 'get_internal_links' ), 10, 2 );
	}
	...
	public function add_internal_links( $content ) {
		...
		$links = apply_filters( 'wpncr_seo_internal_links', array(), $post->ID );
		...
		$links_html = apply_filters( 'wpncr_seo_internal_links_html', $this->format_links( $links ), $links, $post->ID );
	...
	private function format_links( $links ) {
		...
		$html = '<div class="wpncr-related-links">';
	...
	public function add_yoast_schema( $pieces, $context ) {
		...
		if ( ! empty( $related_articles ) ) {
			...
			do_action( 'wpncr_seo_schema_related', $related_articles, $post->ID );
		}
	...
	public function add_rankmath_schema( $data, $json_ld ) {
		...
		do_action( 'wpncr_seo_schema_related', $related, $post->ID );
