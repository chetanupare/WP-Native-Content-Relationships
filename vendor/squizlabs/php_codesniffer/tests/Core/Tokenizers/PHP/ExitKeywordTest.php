...
final class ExitKeywordTest extends AbstractTokenizerTestCase {
	...
	public function testExitIsKeyword( $testMarker, $testContent ) {
		...
		$token      = $this->getTargetToken( $testMarker, array( T_EXIT, T_STRING ), $testContent );
	...
	public static function dataExitIsKeyword() {
		return array(
			'exit as constant'                           => array(
				'testMarker'  => '/* testExitAsConstant */',
				'testContent' => 'exit',
			),
			'die as constant'                            => array(
				'testMarker'  => '/* testDieAsConstant */',
				'testContent' => 'die',
			),
			'exit as constant; mixed case'               => array(
				'testMarker'  => '/* testExitAsConstantMixedCase */',
				'testContent' => 'Exit',
			),
			'die as constant; uppercase'                 => array(
				'testMarker'  => '/* testDieAsConstantUppercase */',
				'testContent' => 'DIE',
			),
			'exit as function call; no parameters'       => array(
				'testMarker'  => '/* testExitAsFunctionCallNoParam */',
				'testContent' => 'exit',
			),
			'die as function call; no parameters'        => array(
				'testMarker'  => '/* testDieAsFunctionCallNoParam */',
				'testContent' => 'die',
			),
			'exit as function call; with parameters'     => array(
				'testMarker'  => '/* testExitAsFunctionCallWithParam */',
				'testContent' => 'exit',
			),
			'die as function call; with parameters'      => array(
				'testMarker'  => '/* testDieAsFunctionCallWithParam */',
				'testContent' => 'die',
			),
			'exit as function call; uppercase'           => array(
				'testMarker'  => '/* testExitAsFunctionCallUppercase */',
				'testContent' => 'EXIT',
			),
			'die as function call; mixed case'           => array(
				'testMarker'  => '/* testDieAsFunctionCallMixedCase */',
				'testContent' => 'dIE',
			),
			'exit as fully qualified function call; with parameters' => array(
				'testMarker'  => '/* testExitAsFQFunctionCallWithParam */',
				'testContent' => 'exit',
			),
			'die as fully qualified function call; no parameters' => array(
				'testMarker'  => '/* testDieAsFQFunctionCallNoParam */',
				'testContent' => 'die',
			),
			'exit as fully qualified constant (illegal)' => array(
				'testMarker'  => '/* testExitAsFQConstant */',
				'testContent' => 'exit',
			),
			'die as fully qualified constant (illegal)'  => array(
	...
	public function testNotExitKeyword( $testMarker, $testContent, $expected = 'T_STRING' ) {
		...
		$token      = $this->getTargetToken( $testMarker, array( T_EXIT, $tokenCode ), $testContent );
		$tokenArray = $tokens[ $token ];
	...
	public static function dataNotExitKeyword() {
		return array(
			'exit not keyword: OO constant use'           => array(
				'testMarker'  => '/* testNotExitOOConstantAccess */',
				'testContent' => 'exit',
			),
			'die not keyword: OO constant use'            => array(
				'testMarker'  => '/* testNotDieOOConstantAccess */',
				'testContent' => 'die',
			),
			'exit not keyword: OO property access'        => array(
				'testMarker'  => '/* testNotExitOOPropertyAccess */',
				'testContent' => 'exit',
			),
			'die not keyword: OO property access'         => array(
				'testMarker'  => '/* testNotDieOOPropertyAccess */',
				'testContent' => 'DIE',
			),
			'exit not keyword: OO method call'            => array(
				'testMarker'  => '/* testNotExitOOMethodCall */',
				'testContent' => 'exit',
			),
			'die not keyword: OO method call'             => array(
				'testMarker'  => '/* testNotDieOOMethodCall */',
				'testContent' => 'die',
			),
			'exit not keyword: OO constant declaration'   => array(
				'testMarker'  => '/* testNotExitOOConstDeclaration */',
				'testContent' => 'exit',
			),
			'die not keyword: OO constant declaration'    => array(
				'testMarker'  => '/* testNotDieOOConstDeclaration */',
				'testContent' => 'die',
			),
			'exit not keyword: OO method declaration'     => array(
				'testMarker'  => '/* testNotExitOOMethodDeclaration */',
				'testContent' => 'Exit',
			),
			'die not keyword: OO method declaration'      => array(
				'testMarker'  => '/* testNotDieOOMethodDeclaration */',
				'testContent' => 'die',
			),
			'exit not keyword: parameter label for named param' => array(
				'testMarker'  => '/* testNotExitParamName */',
				'testContent' => 'exit',
				'expected'    => 'T_PARAM_NAME',
			),
			'die not keyword: parameter label for named param' => array(
				'testMarker'  => '/* testNotDieParamName */',
				'testContent' => 'die',
				'expected'    => 'T_PARAM_NAME',
			),
			'exit not keyword: part of a namespaced name' => array(
				'testMarker'  => '/* testNotExitNamespacedName */',
				'testContent' => 'exit',
			),
			'die not keyword: part of a namespaced name'  => array(
				'testMarker'  => '/* testNotDieNamespacedName */',
				'testContent' => 'die',
			),
			'exit not keyword: global constant declaration (illegal)' => array(
				'testMarker'  => '/* testNotExitConstantDeclaration */',
				'testContent' => 'exit',
			),
			'die not keyword: global constant declaration (illegal)' => array(
				'testMarker'  => '/* testNotDieConstantDeclaration */',
				'testContent' => 'die',
			),
			'exit not keyword: global function declaration (illegal)' => array(
				'testMarker'  => '/* testNotExitFunctionDeclaration */',
				'testContent' => 'exit',
			),
			'die not keyword: global function declaration (illegal)' => array(
